  ...
  async nextUtterance(session: AgentSession, history: Transcript): Promise<{ text: string; end: boolean }> {
    ...
    if (!this.sessionId) {
      ...
      const system = [
        ...
        `This is a real phone call: it ends when someone hangs up. You hang up by appending the exact token ${END_TOKEN} to a line — the only way the call ends on your side. End once the business is genuinely finished: you got what you came for, it's clear you won't, or the other person hung up. Stay in character right to the very end and close the way a real caller would — a quick, natural sign-off like "perfect, that's all I needed, thanks — bye" — with ${END_TOKEN} appended to that line. Do NOT debrief, reveal that this was a test, grade the target, or offer another round; just hang up. Any sign-off line MUST carry ${END_TOKEN}, or the call cannot end.`,
      ].join('\n\n');
